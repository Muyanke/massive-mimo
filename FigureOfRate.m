...
SNR=0:5:20;%单位dB  SNR=P/sigma^2;
Rate1=zeros(size(SNR,2));%等功率分配速率
Rate2=zeros(size(SNR,2));%注水算法速率
for i=1:size(SNR,2)

Sigma_n=(10.^(SNR(i)./10))./(K*Ns).*Sigma_n;
...
%得到所有的功率分配矩阵
Gama_k1=zeros(Ns,Ns,K);%等功率
Gama_k2=zeros(Ns,Ns,K);%注水
Rate_k1=zeros(K,1);
Rate_k2=zeros(K,1);
for k=1:K
    Gama_k1(:,:, k)=eye(Ns);%等功率
    Gama_k2(:,:, k)=diag(Gama(2*k-1:2*k,1));%注水
    Rate_k1(k,1)=log2(det(eye(Ns)+((10.^(SNR(i)./10))./(K*Ns)).*Gama_k1(:,:, k).*Sigma(:,:,k).^2));
    Rate_k2(k,1)=log2(det(eye(Ns)+((10.^(SNR(i)./10))./(K*Ns)).*Gama_k2(:,:, k).*Sigma(:,:,k).^2));
end
Rate1(i)=sum(Rate_k1);
Rate2(i)=sum(Rate_k2);
end
figure;
plot(SNR,Rate1,'k-',SNR,Rate2,'r-');
xlabel('SNR0(dB)');
ylabel('Sum Rate')

title('等功率速率分配和注水功率分配速率对比图')
